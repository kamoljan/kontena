...
class AccessToken
  ...
  set_callback :save, :before do |doc|
    ...
    doc.token ||= AuthProvider.encrypt(doc.token_plain)
    doc.refresh_token ||= AuthProvider.encrypt(doc.refresh_token_plain)
  end
  ...
  class << self
    ...
    def find_by_refresh_token_and_mark_used(refresh_token)
      AccessToken.where(
        refresh_token: AuthProvider.encrypt(refresh_token),
        deleted_at: nil,
        internal: true
    ...
    def find_internal_by_access_token(access_token)
      AccessToken.where(
        access_token: AuthProvider.encrypt(access_token),
        deleted_at: nil,
        internal: true
